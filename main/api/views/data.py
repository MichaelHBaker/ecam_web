...
class DataProcessor:
    ...
    def analyze_json_structure(self, file_obj: File, encoding: str) -> Dict:
        """
        ...
        try:
            ...
            if is_array:
                ...
                array_path = self._find_array_in_json(data)
                sample_data = self._get_json_by_path(data, array_path)[0] if array_path else {}
                path = array_path
            ...
            columns = [
                {
                    ...
                    'index': idx,
                    'sample_values': [
                        self._get_json_by_path(item, key) 
                        for item in (data if is_array else self._get_json_by_path(data, path))[:3]
                    ]
                }
            ...
            raise ValidationError(f"JSON analysis failed: {str(e)}")
            
    def analyze_xml_structure(self, file_obj: File, encoding: str) -> Dict:
        """
        Analyze XML file structure
        """
        try:
            # For XML, we'll return a simpler structure since full XML parsing
            # would require additional libraries. In practice, you'd want to use
            # a proper XML parser like lxml.
            content = file_obj.read().decode(encoding)
            file_obj.seek(0)  # Reset file pointer
            ...
            return {
                'path': f'//{repeated_tag}' if repeated_tag else '',
                'data_start_line': 0,
                'root_element': root_tag,
                'repeated_element': repeated_tag
            }
            
        except Exception as e:
            raise ValidationError(f"XML analysis failed: {str(e)}")
            
    def _find_array_in_json(self, data: Dict, path: str = '') -> str:
        """Helper method to find first array in JSON object"""
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                if isinstance(value, dict):
                    result = self._find_array_in_json(value, new_path)
                    if result:
                        return result
        return ''
        
    def _get_json_by_path(self, data: Dict, path: str):
        """Helper method to get value from JSON by dot notation path"""
        if not path:
            return data
        parts = path.split('.')
        current = data
